i82586_version	equ	3
;History:285,1 

  ifndef SEG586				;segment of 586's memory.
SEG586	equ	0
  endif
...
CBCONF_FLAGS	label	byte
	DW	2		; flags, set to 1 for promiscuous
...
wait_for_transmit_to_complete:
	mov	bx,es:[TCBPTR].tcb_status
	test	bx,4000h
	jnz	wait_for_transmit_to_complete
	mov	es:[TCBPTR].tcb_status,0;zero status wd
	test	bx,720h			;any really bad errors?
	jne	send_pkt_bad		;yes, give up.
;
...
found_mem_msg	db	"K memory found.",CR,LF,'$'
...
;now put the location of the first rbd into the first fd.
  ...
  if 0	;this message only confuses users.
